  ...
  asyncData (context) {
    ...
    data:function(){
      return{
          ...
          machineCode:"LN034",   //asyncData で当該マシンコードがレンダリング前に格納される。
          machineCodeArry:["LN034","MC031"],
...
Line_options: {
  ...
  methods:{
            ...
            if(person.machine==this.machineCode){ //対象機械のみ
                ...
                if(TgtDStartD != TgtDEndD || person.startTime=="999"){
                  ...
                  for(let i=5;i>0;i--){
                    ...
                    forStart=new Date(TgtDEnd.setSeconds(TgtS-i)); //TgtDEndの時間からi秒を引く（初回は5秒）
                    ...
                    for(const elem of machineCodeArry){
                      console.log("現在のマシンは、"+elem);
                      // this.chartData.push([elem,nowForStatus,forStart,forEnd]);
                 ...
                 //============================================================================================
                  ...
                  let nowStatus;
                  if(person.status=="RUN1" ||person.status=="ERR1"||person.status=="DDR1"||person.status=="KKT1"){
    ...
    ObiArryPush:function(h,nowStatus,nowSeconds,TgtDStart,TgtDEnd,GraphTitle){
      ...
      // this.chartData.push(["MC031",nowStatus,new Date(TgtDStart),new Date(TgtDEnd)]);
      // console.log(this.chartData);
